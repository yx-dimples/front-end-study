...
exports.findById = function (id, callback) {
  fs.readFile(dbPath, function (err, data) {
    ...
    //1.获取旧数据
    const stus = JSON.parse(data.toString()).stus
    //2.通过js的find方法获取指定id对象
    const stu = stus.find(function (item) {
      return item.id == id
    })
    //触发执行回调函数
    callback(null, stu)
...
exports.add = function (stu, callback) {
  fs.readFile(dbPath, function (err, data) {
    ...
    stus.push(stu)
    //3.入库（把字符串保存到文件中）
    const fileData = JSON.stringify({stus: stus})
...
exports.updateById = function (stu, callback) {
  fs.readFile(dbPath, function (err, data) {
    ...
    const stus = JSON.parse(data.toString()).stus
    //2.更新旧数据
    const updateId = stus.findIndex(function (item) {
...
exports.deleteById = function (id, callback) {
  fs.readFile(dbPath, function (err, data) {
    ...
    const stus = JSON.parse(data.toString()).stus
    //2.删除数据
    const deleteId = stus.findIndex(function (item) {
      //获取删除数据数组下标
      return item.id == id
    })
